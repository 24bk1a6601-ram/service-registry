...
async function startServer() {
  ...
  app.post('/api/blockchain/agents', (req, res) => {
    const { name, organization, metadataURI, owner } = req.body;
    if (!name || !owner) {
      return res.status(400).json({ error: 'Missing name or owner address' });
    }
    const newAgent = blockchainService.registerAgent({ name, organization, metadataURI, owner });
  ...
  app.post('/api/blockchain/services', (req, res) => {
    const { agentId, name, description, category, pricePerRequestWei, priceFormatted, slaGuarantee, owner } = req.body;
